...
let ballState = {
    ...
    spin: new THREE.Vector3(0, 0, 0), // x: topspin/backspin, y: sidespin, z: gyrospin
    active: true,
    lastHitBy: null
};
...
function createPaddles() {
    ...
    // Blade
    const blade = new THREE.Mesh(paddleGeometry, paddleMaterial);
    paddle.add(blade);
    
    // Black rubber side
    const blackRubber = new THREE.Mesh(
        paddleGeometry,
...
function updateBall(deltaTime) {
    ...
    const dt = Math.min(deltaTime, 0.02); // Cap delta time
    ...
    // Apply spin effects (Magnus force)
    ...
    // Update ball glow color based on spin
    updateBallGlow();
}
...
function checkTableCollision() {
    ...
    if (Math.abs(ballState.position.x) < halfWidth &&
        ...
        if (ballState.position.y - PHYSICS.ballRadius < tableTop && ballState.velocity.y < 0) {
            ...
            playSound('hit');
            createBounceParticles(ballState.position.clone());
        }
    }
}
...
function checkNetCollision() {
    ...
    // Check if ball passes through net area
...
function checkPaddleCollision() {
    ...
    const paddleBox = new THREE.Box3().setFromCenterAndSize(
        ...
        new THREE.Vector3(PHYSICS.paddleWidth, 0.05, PHYSICS.paddleHeight)
    );
    
    const ballSphere = new THREE.Sphere(ballState.position, PHYSICS.ballRadius);
    
    if (paddleBox.intersectsSphere(ballSphere) && ballState.velocity.z > 0) {
        handlePaddleHit(paddleState, 'player');
    }
    
    // Opponent paddle collision
    const opponentBox = new THREE.Box3().setFromCenterAndSize(
        opponentState.position,
        new THREE.Vector3(PHYSICS.paddleWidth, 0.05, PHYSICS.paddleHeight)
    );
    
    if (opponentBox.intersectsSphere(ballSphere) && ballState.velocity.z < 0) {
...
function handlePaddleHit(paddleData, player) {
    ...
    const hitZ = (ballState.position.z - paddleData.position.z) / (PHYSICS.paddleHeight / 2);
    ...
    if (isPlayer) {
        ...
        const paddleVelY = paddleState.velocity.y || 0;
        const paddleVelZ = paddleState.velocity.z;
        
        // Topspin/Backspin from vertical paddle movement
        // Moving paddle forward = topspin, backward = backspin
        const topspinAmount = -paddleVelZ * 2;
        
        // Sidespin from horizontal paddle movement
...
function checkOutOfBounds() {
    ...
    // Ball fell off sides or too low
    if (Math.abs(ballState.position.x) > 3 || ballState.position.y < -1) {
        // Determine who gets the point based on last hit
...
function updateBot(deltaTime) {
    ...
    if (botReactionDelay <= 0) {
        ...
        if (ballState.velocity.z < 0) {
            ...
            let predictedY = ballState.position.y + ballState.velocity.y * timeToReach + 
                            0.5 * PHYSICS.gravity * timeToReach * timeToReach;
...
function updateCamera(deltaTime) {
    // Camera follows ball slightly and paddle position
    ...
    // Dynamic camera height based on ball position
...
function updateBallGlow() {
    ...
    if (spinMagnitude > 0.2) {
        let color;
        if (ballState.spin.x > 0.2) {
            color = new THREE.Color(0xff6600); // Topspin - orange
        } else if (ballState.spin.x < -0.2) {
            color = new THREE.Color(0x00ff66); // Backspin - green
        } else if (Math.abs(ballState.spin.y) > 0.2) {
            color = new THREE.Color(0xffff00); // Sidespin - yellow
        } else {
...
function createBounceParticles(position) {
    createHitParticles(position, 0xffffff);
}
...
function updateEffects(deltaTime) {
    ...
    // Draw ball trail
    // (Could be implemented with THREE.Line for performance)
}
